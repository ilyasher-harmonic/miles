...
def assert_the_take_overs_replaced_only_the_script(
...
def assert_only_the_orchestration_side_restarted(evidence: HotRestartEvidence, *, num_restarts: int) -> None:
    ...
    replaced = _compute_workloads_whose_pods_were_replaced(evidence.snapshots)
    ...
    rolled = _compute_workloads_whose_template_changed(evidence.snapshots)
...
def _compute_workloads_whose_pods_were_replaced(snapshots: Sequence[ClusterSnapshot]) -> dict[str, list[str]]:
