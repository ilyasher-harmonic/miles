...
name `${NAMESPACE}`, which the chart replaces with the namespace it is installed into; no other
variable is accepted. Keep it in `runsRoot`, or two namespaces running the same run ID would share
one directory of run artifacts.
